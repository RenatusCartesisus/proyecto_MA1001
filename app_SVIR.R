...
library(dplyr)
library(tidyr)
library(deSolve) # Para resolver las ODE
...
###########################################################################
# INTERFAZ (UI) - con nuevo panel para Modelo Determinista
###########################################################################
...
ui = page_sidebar(
  ...
  navset_card_pill(
    ...
    nav_panel("Modelo Determinista",
      plotOutput("deterministic_plot", height = "650px")
    )
  )
)
...
server = function(input, output, session) {
  ...
  ###########################################################################
  # Lógica del Modelo Determinista (ODE)
  ###########################################################################
  
  # Definición del modelo SVIR con ODEs
  svir_ode_model <- function(time, state, parameters) {
    with(as.list(c(state, parameters)), {
      N <- S + V + I + R
      
      # Las tasas de cambio
      # Asumo que β_v = β, es decir, vacunados se infectan con la misma tasa
      dS <- omega * R - (beta * S * I) / N - nu * S
      dV <- nu * S - (beta * V * I) / N
      dI <- (beta * S * I) / N + (beta * V * I) / N - gamma * I
      dR <- gamma * I - omega * R
      
      return(list(c(dS, dV, dI, dR)))
    })
  }
  
  deterministic_data <- eventReactive(input$simulate, {
    # This code will only run when input$simulate is clicked
    params <- c(
      beta  = input$beta,
      gamma = input$miu,
      nu    = input$nu,
    ...
    validate(
      need(S0 >= 0, "El número de infectados iniciales no puede superar la población total.")
    )

    initial_state <- c(S = S0, V = V0, I = I0, R = 0)
    times <- seq(0, input$T, by = 0.1)
    
    ode(y = initial_state, times = times, func = svir_ode_model, parms = params)
  })
  
  output$deterministic_plot <- renderPlot({
    # Wait for the simulation button to be clicked and get data
    solution <- deterministic_data()
    ...
    ggplot(solution_df, aes(x = time, y = Poblacion, color = Compartimento)) +
      geom_line(linewidth = 1.5) +
      scale_color_manual(
        values = c("S" = color_s, "V" = color_v, "I" = color_i, "R" = color_r),
        name = "Compartimento"
      ) +
      labs(
        title = "Evolución del Modelo SIRV Determinista",
        x = "Tiempo (días)",
